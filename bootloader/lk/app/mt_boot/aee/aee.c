...
static int mrdump_emmc_output(const struct mrdump_control_block *kparams, uint64_t total_dump_size)
{
	...
	switch (fstype) {
		case MRDUMP_FS_EXT4:
			...
			mrdump_ext4_output(kparams, total_dump_size, mrdump_dev_emmc_ext4());
			break;
		case MRDUMP_FS_VFAT:
			voprintf_info("Output to VFAT Partition\n");
			mrdump_vfat_output(kparams, total_dump_size, mrdump_dev_emmc_vfat());
			break;
		case MRDUMP_FS_NULL:
		default:
...
static void kdump_ui(struct mrdump_control_block *mrdump_cblock)
{
	...
	switch (mrdump_cblock->machdesc.output_device) {
		...
		case MRDUMP_DEV_SDCARD:
			mrdump_sdcard_output(mrdump_cblock, mrdump_mem_size());
			break;
#endif
		case MRDUMP_DEV_EMMC:
			mrdump_emmc_output(mrdump_cblock, mrdump_mem_size());
			break;
