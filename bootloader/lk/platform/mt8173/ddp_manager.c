...
static ddp_path_handle_t g_extd_handle;
...
disp_path_handle dpmgr_create_path(DDP_SCENARIO_ENUM scenario, cmdqRecHandle cmdq_handle)
{
	...
	if (scenario == DDP_SCENARIO_SUB_DISP) {
		path_handle  = &g_extd_handle;
	} else {
		path_handle  = &g_handle;
	}

	DISP_LOG_I("%s handle 0x%x on scenario %s\n",__func__,(unsigned int)path_handle,ddp_get_scenario_name(scenario));
	if (NULL != path_handle) {
...
int dpmgr_wait_event_timeout(disp_path_handle dp_handle, DISP_PATH_EVENT event, int timeout)
{
	...
	if (wq_handle->init) {
		...
		if (ret > 0) {
			...
			DISP_LOG_E("wait event %d timeout ret %d on scenario %d \n", event,ret,handle->scenario);
