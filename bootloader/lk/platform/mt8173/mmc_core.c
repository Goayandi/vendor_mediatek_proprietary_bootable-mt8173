...
int mmc_go_irq_state(struct mmc_host *host, struct mmc_card *card)
{
	...
	if (!(card->csd.cmdclass & MMC_CCC_IO_MODE)) {
		dprintf(CRITICAL, "[SD%d] Card doesn't support I/O mode for IRQ state\n", host->id);
	...
	if (mmc_card_sd(card)) {
		...
		*/
		...
		switch (card->csd.mmca_vsn) {
			...
			default:
				dprintf(CRITICAL, "[SD%d] Unknown MMCA version %d\n",
	...
	if (mmc_card_sd(card)) {
		...
		switch (csd_struct) {
			...
			case 1:
				...
				break;
			default:
				dprintf(CRITICAL, "[SD%d] Unknown CSD ver %d\n", mmc_card_id(card), csd_struct);
				return MMC_ERR_INVALID;
		}
	} else {
		/*
...
static void mmc_decode_ext_csd(struct mmc_card *card)
{
	...
	if ((ext_csd[EXT_CSD_CARD_TYPE] & EXT_CSD_CARD_TYPE_DDR_52_1_2V) ||
		...
		dprintf(CRITICAL, "[SD%d] MMCv4 but HS unsupported\n", card->host->id);
	}
	...
	do {
		...
		if (err != MMC_ERR_NONE) {
			if (msdc_abort_handler(host, 1))
				dprintf(CRITICAL, "[SD%d] data abort failed\n",host->id);
...
static int __mmc_do_erase(struct mmc_host *host,struct mmc_card *card,u64 start_addr,u64 len)
{
	...
	if (end_addr/card->blklen > card->nblks) {
		dprintf(CRITICAL, "[MSDC%d]Erase address out of range! start<0x%llx>,len<0x%llx>,card_nblks<0x%x>\n",host->id,start_addr,len,card->nblks);
		return MMC_ERR_INVALID;
	}
	if ((err = mmc_erase_start(card, start_addr))) {
		dprintf(CRITICAL, "[MSDC%d]Set erase start addrees 0x%llx failed,Err<%d>\n",host->id,start_addr,err);
		goto out;
	}
	if ((err = mmc_erase_end(card, end_addr))) {
		dprintf(CRITICAL, "[MSDC%d]Set erase end addrees 0x%llx + 0x%llx failed,Err<%d>\n",host->id,start_addr,len,err);
		goto out;
	}
	if ((err = mmc_erase(card, MMC_ERASE_TRIM))) {
		dprintf(CRITICAL, "[MSDC%d]Set erase <0x%llx - 0x%llx> failed,Err<%d>\n",host->id,start_addr,start_addr + len,err);
		goto out;
	}
	dprintf(CRITICAL, "[MSDC%d]0x%llx - 0x%llx Erased\n", host->id,start_addr,start_addr + len);
out:
	return err;
}
int mmc_do_erase(int dev_num,u64 start_addr,u64 len,u32 part_id)
{
	...
	u32 err;
	if ((!card) || (!host) ) {
		dprintf(INFO, "[mmc_do_erase] card or host is NULL\n");
		return MMC_ERR_INVALID;
	}
	if (!len ) {
		dprintf(CRITICAL, "[MSDC%d] invalid erase size! len<0x%llx>\n",host->id,len);
		return MMC_ERR_INVALID;
	}
	if ((start_addr % card->blklen) || (len % card->blklen)) {
		dprintf(CRITICAL, "[MSDC%d] non-alignment erase address! start<0x%llx>,len<0x%llx>,card_nblks<0x%x>\n",host->id,start_addr,len,card->nblks);
		return MMC_ERR_INVALID;
	}
	...
	if (mmc_virt_to_phys(s_blknr, &s_blknr, &s_pid) != MMC_ERR_NONE) {
		dprintf(CRITICAL, "[MSDC%d] s_addr trans error blknr<0x%x>\n",host->id,s_blknr);
		return MMC_ERR_INVALID;
	}
	if (mmc_virt_to_phys(e_blknr, &e_blknr, &e_pid) != MMC_ERR_NONE) {
		dprintf(CRITICAL, "[MSDC%d] e_addr trans error blknr<0x%x>\n",host->id,e_blknr);
		return MMC_ERR_INVALID;
	}
	s_pid_o = s_pid;
	...
	if ((err =  __mmc_do_erase(host, card, start_addr, len))) {
		dprintf(CRITICAL, "[MSDC%d] mmc erase failed.error <%d> \n",host->id, err);
		return err;
	}
#endif
	return err;
...
int mmc_set_bus_width(struct mmc_host *host, struct mmc_card *card, int width)
{
	...
	if (mmc_card_sd(card)) {
		...
		if (err != MMC_ERR_NONE) {
			dprintf(CRITICAL, "[SD%d] Switch to bus width(%d) failed\n", host->id, arg);
			goto out;
		}
...
static int mmc_block_read_without_switch(int dev_num, unsigned long blknr, u32 blkcnt, unsigned long *dst);
static int mmc_block_write_without_switch(int dev_num, unsigned long blknr, u32 blkcnt, unsigned long *src);
...
int mmc_part_read(struct mmc_card *card, u8 partno, unsigned long blknr, u32 blkcnt, unsigned long *dst)
{
	...
	err = mmc_block_read_without_switch(host->id, blknr, blkcnt, dst);
...
int mmc_part_write(struct mmc_card *card, u8 partno, unsigned long blknr, u32 blkcnt, unsigned long *src)
{
	...
	/* write block to this partition */
	err = mmc_block_write_without_switch(host->id, blknr, blkcnt, src);
...
int mmc_dev_bread(struct mmc_card *card, unsigned long blknr, u32 blkcnt, u8 *dst)
{
	...
	do {
		...
		if (err == MMC_ERR_BADCRC || err == MMC_ERR_ACMD_RSPCRC || err == MMC_ERR_CMD_RSPCRC) {
			...
			dprintf(CRITICAL, "[SD%d] Fail to tuning,%s",host->id,(err == MMC_ERR_CMDTUNEFAIL)?"cmd tune failed!\n":"read tune failed!\n");
			break;
		}
	} while (retry--);
...
int mmc_block_read(int dev_num, unsigned long blknr, u32 blkcnt, void *dst, u32 part_id)
{
	...
	//dprintf(INFO, "[SD%d] virt addr:%ld\n",dev_num,blknr);
	//mmc_virt_switch(blknr, &blknr);
	//dprintf(INFO, "[SD%d] phys addr:%ld\n",dev_num,blknr);
	if (!blkcnt)
		return MMC_ERR_NONE;

	if (blknr * (blksz / MMC_BLOCK_SIZE) > card->nblks) {
		dprintf(CRITICAL, "[SD%d] Out of block range: blknr(%lu) > sd_blknr(%u)\n",
...
int mmc_block_write(int dev_num, unsigned long blknr, u32 blkcnt, const void *src, u32 part_id)
{
	...
	//dprintf(INFO, "virt addr:%ld\n",dev_num,blknr);
	//mmc_virt_switch(blknr, &blknr);
	//dprintf(INFO, "phys addr:%ld\n",dev_num,blknr);
	if (!blkcnt)
		return MMC_ERR_NONE;
...
int mmc_init_mem_card(struct mmc_host *host, struct mmc_card *card, u32 ocr)
{
	...
	if (err != MMC_ERR_NONE) {
		dprintf(CRITICAL, "[SD%d] Fail in SEND_OP_COND cmd\n", id);
		goto out;
	}
	...
	if (err != MMC_ERR_NONE) {
		dprintf(CRITICAL, "[SD%d] Fail in decode csd\n", id);
		goto out;
	}
	...
	if (!(mmc_card_mmc(card) && mmc_card_ddr(card))) {
		...
		if (err != MMC_ERR_NONE) {
			dprintf(CRITICAL, "[SD%d] Fail in set blklen cmd\n", id);
			goto out;
		}
	}
...
static int msdc_ett_offline_to_ub(struct mmc_host *host, struct mmc_card *card)
{
	...
	dprintf(INFO, "msdc_ett_offline_to_ub: size<%d> m_id<0x%x>\n", size, m_id);

	for (i = 0; i < size; i++) {
		dprintf(INFO, "msdc <%d> <%s> <%s>\n", i, g_mmcTable[i].pro_name, pro_name);

		if ((g_mmcTable[i].m_id == m_id) && (!strncmp(g_mmcTable[i].pro_name, pro_name, 6))) {
			dprintf(INFO, "msdc ett index<%d>: <%d> <%d> <0x%x> <0x%x> <0x%x>\n", i,
			...
			MSDC_SET_FIELD(MSDC_IOCON, MSDC_IOCON_R_D_SMPL, g_mmcTable[i].d_smpl);
	...
	if (ret) dprintf(CRITICAL, "msdc failed to find\n");
	return ret;
}
#endif
int mmc_init_card(struct mmc_host *host, struct mmc_card *card)
{
	...
	if (err != MMC_ERR_NONE) {
		...
		if (err != MMC_ERR_NONE) {
			dprintf(CRITICAL, "[SD%d] Fail in SEND_IF_COND cmd\n", id);
			goto out;
		}
...
#ifdef FPGA_PLATFORM
//#define MTK_MSDC_LK_TEST
#endif
#ifdef MTK_MSDC_LK_TEST
...
static void emmc_r_w_compare_test()
{
	...
	dprintf(INFO, "[LK][%s:%d] finish write\n", __func__, __LINE__);
	...
	dprintf(INFO, "[LK][%s:%d] finish read\n", __func__, __LINE__);

	/* compara */
	for (i = 0; i < LK_MMC_TEST_SIZE; i++) {
		if (g_mmc_buf[i] != 0x5a) {
			dprintf(CRITICAL, "[LK][%s:%d]mmc simple r/w compare failed(%d is %d, not 0x5a)\n", __func__, __LINE__, i, g_mmc_buf[i]);
			break;
		}
	}
...
int mmc_legacy_init(int verbose)
{
	...
	dprintf(CRITICAL, "[%s]: msdc%d start in LK\n", __func__, id);
	BUG_ON(id >= NR_MMC);
	...
	card = &sd_card[id];
	bdev = &sd_dev[id];
	dprintf(CRITICAL, "[%s]: msdc%d, start mmc_init_host() in LK\n", __func__, id);
	err = mmc_init_host(host, id);

	if (err == MMC_ERR_NONE) {
		dprintf(CRITICAL, "[%s]: msdc%d start mmc_init_card() in LK\n", __func__, id);
		err = mmc_init_card(host, card);
	}
	...
	if (err == MMC_ERR_NONE && !boot_dev_found) {
		...
		mmc_test(0, NULL);
#endif
		dprintf(CRITICAL, "[%s]: msdc%d mmc_addr_trans_tbl_init()\n", __func__, id);
		...
		if (host->boot_type == RAW_BOOT) {
			...
			boot_dev_found = 1;
			dprintf(CRITICAL, "[%s]: msdc%d boot device found\n", __func__, id);
		} else if (host->boot_type == FAT_BOOT) {
#if (CONFIG_COMMANDS & CFG_CMD_FAT)
			if (0 == fat_register_device(bdev, 1)) {
				boot_dev_found = 1;
				dprintf(CRITICAL, "[%s]: msdc%d FAT partition found\n", __func__, id);
			}
#endif
		}
	}
	...
	dprintf(CRITICAL, "[%s]: msdc%d, use hc erase size\n", __func__, id);
	...
	dprintf(CRITICAL, "[%s]: msdc%d init done in LK\n", __func__, id);
	return err;
}
#endif
