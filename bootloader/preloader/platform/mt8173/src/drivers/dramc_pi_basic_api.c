...
//        mcSHOW_ERR_MSG(("context is NULL\n"));
...
#ifdef LEW_SIMULATION
	 p->channel = CHANNEL_A;
      ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x1e0), 0x220fffff);
      ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x23c), 0x220fffff);
    ...
    if ((p->package == PACKAGE_POP)||(p->package == PACKAGE_SBS) )//temply 1
    {
        //=================== Integration Part (TOP) =======================
        // Enable MTCMOS power ack switch before ACK_POP_BYP and ACK_SBS_BYP to avoid glitch
        // LDO_PHY2POP_NDIS=1 is better from LDO designer
	...
	#else
	     #ifdef LEW_SIMULATION
            ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x640), 0x09008023);
	     #else
	     ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x640), 0x09008024);
	     #endif
	#endif
...
#endif
        #ifdef LEW_SIMULATION
        //ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x690), 0x01008004); //SPM control
        ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x690), 0x09008004); //SPM control clock enable
        #else
        ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x690), 0x09008004);
        #endif
        p->channel = CHANNEL_B;
...
//#else
        ...
        mcCLR_BIT(u4value, 15);
        #ifdef LEW_SIMULATION
        mcCLR_BIT(u4value, 0); //SPM control clock enable
        #endif
        ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x640), u4value);
        //ucstatus |= ucDram_Register_Read(mcSET_DRAMC_REG_ADDR(0x690), &u4value);
        //mcCLR_BIT(u4value, 15);
 ...
 //   #endif
        ...
        #ifndef LEW_SIMULATION
        mcSET_BIT(u4value, 0);//SPM control clock enable
        #endif
        ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x640), u4value);
        #ifndef LEW_SIMULATION
        ucstatus |= ucDram_Register_Read(mcSET_DRAMC_REG_ADDR(0x690), &u4value);
...
//        mcSHOW_ERR_MSG(("context is NULL\n"));
    ...
    if ((p->package == PACKAGE_POP)||(p->package == PACKAGE_SBS) )  //temply 1==1
    {        
    #if 0
        //=================== Integration Part (TOP) =======================
        // Enable MTCMOS power ack switch before ACK_POP_BYP and ACK_SBS_BYP to avoid glitch
...
//#else
 ...
 //   #endif
	...
	VCOK_Chb_Mempll2 = ((u4value >> 8) & 0x3f) | (u4value & 0x40);
        ...
        #else
                ucstatus |= ucDram_Register_Read(mcSET_DRAMC_REG_ADDR(0x3e0), &u4value);
	VCOK_Chb_Mempll2 = (u4value >> 16) & 0x7f;
	VCOK_Chb_Mempll3 = (u4value >> 8) & 0x7f;
        mcSHOW_DBG_MSG3(("0x3e0=0x%x, VCOK_Cha_Mempll2=0x%x, VCOK_Cha_Mempll3=0x%x \n", 
        	u4value, VCOK_Chb_Mempll2, VCOK_Chb_Mempll3));
    ...
    //    VCOK_05PHY_Mempll3 = (u4value >> 8) & 0x7f; 
     ...
     //   	u4value, VCOK_05PHY_Mempll2, VCOK_05PHY_Mempll3));
        ...
        ucstatus |= ucDram_Register_Read(mcSET_DRAMC_REG_ADDR(0x3e0), &u4value);
        VCOK_Chb_Mempll4 = ((u4value >> 8) & 0x3f) | (u4value & 0x40);
        mcSHOW_DBG_MSG3(("0x3e0=0x%x, VCOK_Cha_Mempll4=0x%x\n", u4value, VCOK_Chb_Mempll4));
...
//	mcDELAY_US(13);
    ...
    */
        ...
        ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x640), 0x090000B1);
      
        ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x640), 0x090000B2);//SPM control clock enable
...
{
	    U8 ucstatus = 0;
      U32 u4value, u4WL, u4MR2;

#ifdef COMBO_MCP        
#if 0
		u4MR2 = (emi_set->iLPDDR3_MODE_REG_2 >> 16) & 0xFF;
    #else
		u4MR2 = (DramcGetMR2ByFreq(mt_get_dram_freq_setting()) >> 16) & 0xFF;
	#endif
#else
		u4MR2 = (LPDDR3_MODE_REG_2 >> 16) & 0xFF;
#endif 
		switch(u4MR2&0x0F)
		{
			case 0x07:	// RL9 WL5
		...
		print("DramcPreInit: MR2 = 0x%x, WL = 0x%x\n", u4MR2, u4WL); 
...
#ifdef TX_ADV_1T
	ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x400), 0x00000000);//TXDLY_CS, TXDLY_CS1
	ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x404), 0x00302000);//TXDLY_DQSGate, TXDLY_DQSGate_P1
	ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x408), 0x00000000);//TXLDY_RA*
	ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x40c), 0x00000000);//TXLDY_RA*
	...
	ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x42c), 0x005500ff);     
      ...
      // CA phase select
#ifdef CS_ADV      
      ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x1e0), 0x221fffff); // edward adjust CS 
#else      
      ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x1e0), 0x220fffff);
#endif
...
#ifdef CS_ADV      
      ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x23c), 0x221fffff); // edward adjust CS 
#else
      ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x23c), 0x220fffff);
#endif      

    #if 0  // mark for MT8173 bring up
      // LDO setting
...
#endif
{
	...
	else
	{
        ...
        ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x0d8), 0x40100110);
	...
	else
	{
        ...
        ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x124), 0xc0060011);
        #else
...
//        ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x094), 0x40404040);
	...
	dqsi_gw_dly_coarse_factor_handler(p, 28);
        // Reg.94h
	dqsi_gw_dly_fine_factor_handler(p, 56);       
        // Reg.118h[3:0],  TXDLY_R1DQSGATE:0x418[6:4],  TXDLY_R1DQSGATE_P1: 0x418[10:8]
        // dly_R1DQSGATE: 0x418[1:0], dly_R1DQSGATE_P1: 0x418[3:2]
	dqsi_gw_dly_coarse_factor_handler_rank1(p, 28);
        // Reg.98h
	dqsi_gw_dly_fine_factor_handler_rank1(p, 56);   
	
	// DLE calibration	
	// Reg.7ch[6:4]
	// Reg.e4h[4] 		DATLAT3
	// Reg.f0h[25]		DATLAT4
	dle_factor_handler(p, 20);		
	#endif
	...
	if (p->channel==CHANNEL_A)
	{
		...
		{
			// DQS RX input delay
			ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x018), 0x0B0C0808);
			ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x01c), 0x0B0C0808);		// Need to adjust further
			// DQ input delay
			ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x210), 0x01020201);
			ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x214), 0x02000200);
			ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x218), 0x03040400);
			ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x21c), 0x03040303);
			ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x220), 0x03050300);
			ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x224), 0x05060606);
			ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x228), 0x04060806);
			ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x22c), 0x00010303);
		}
		else
		{
			// DQS RX input delay
		...
		// DQ output delay
		ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x200), 0x99998898);
		ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x204), 0x88888987);
		ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x208), 0x88888876);
		ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x20c), 0x67778888);
		// DQM output delay
		ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x010), 0x00007788);
		// DQS output delay
		ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x014), 0x7677);
		#endif
	}
	else
	{
		...
		{
			// DQS RX input delay
			ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x018), 0x080C0A0A);
			ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x01c), 0x080C0A0A);	// Need to adjust further.
			// DQ input delay
			ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x210), 0x04040303);
			ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x214), 0x05020200);
			ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x218), 0x04050400);
			ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x21c), 0x04040404);
			ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x220), 0x05050400);
			ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x224), 0x06070606);
			ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x228), 0x02030203);
			ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x22c), 0x00010001);
		}
		else
		{
			// DQS RX input delay
		...
		// TX per-bit calibration.
		// DQ output delay
		ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x200), 0x11212222);
		ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x204), 0x22222221);
		ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x208), 0x21111100);
		ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x20c), 0x01001111);
		// DQM output delay
		ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x010), 0x00000011);
		// DQS output delay
		ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x014), 0x0000);	
		#endif
	}
	#endif
...
#else
//#define PLLGRPREG_SIZE	30       //define in dramc_register.h
#define VCOK_CHB_MEMPLL2		21
#define VCOK_CHB_MEMPLL3		22
#define VCOK_CHB_MEMPLL4		23
#define VCOK_CHA_MEMPLL2		18
#define VCOK_CHA_MEMPLL3		19
#define VCOK_CHA_MEMPLL4		20
//#define VCOK_05PHY_MEMPLL2		15
//#define VCOK_05PHY_MEMPLL3		16
//#define VCOK_05PHY_MEMPLL4		17
#define MEMPLL_SETTING_START 	0
#define MEMPLL_SETTING_STOP 	5
#define MEMPLL_SETTING_ENABLE_START 	24
#define MEMPLL_SETTING_ENABLE_STOP 	29
#define MPLL_PCM				8
#endif
const U32 PLL_RegAddr[PLLGRPREG_SIZE] =
{
...
CHA_DDRPHY_BASE +0x614, // 0xD3040300  //0
CHA_DDRPHY_BASE +0x620, // 0xD3040300
CHA_DDRPHY_BASE +0x62C, // 0xD3040300

CHB_DDRPHY_BASE +0x614, // 0xD3040300  //3
CHB_DDRPHY_BASE +0x620, // 0xD3040300
CHB_DDRPHY_BASE +0x62C, // 0xD3040300

  0x1020928C, // 0x00000003   //6
  0x1020928C, // 0x00000001
  0x10209284, // 0x800FAB52
  0x10209284, // 0x000FAB52
  0x10209280, // 0x00010100   
  0x10209280, // 0x00010101   //11

CHA_DDRPHY_BASE +0x618, // 0x4E00C004 //12
CHA_DDRPHY_BASE +0x624, //0x4E00C003
CHA_DDRPHY_BASE +0x630, //0x4E00C002

CHB_DDRPHY_BASE +0x618, //0x4E00C004  //15
CHB_DDRPHY_BASE +0x624, //0x4E00C003
CHB_DDRPHY_BASE +0x630, //0x4E00C002


CHA_DDRPHY_BASE +0x61C, //0x0002280B  //18
CHA_DDRPHY_BASE +0x628, //0x0002280B
CHA_DDRPHY_BASE +0x634, // 0x0000280B

CHB_DDRPHY_BASE +0x61C, // 0x0002280B  //21
CHB_DDRPHY_BASE +0x628, // 0x0002280B
CHB_DDRPHY_BASE +0x634, // 0x0000280B

CHA_DDRPHY_BASE +0x614, // 0xD3040301  //24
CHA_DDRPHY_BASE +0x620, // 0xD3040301
CHA_DDRPHY_BASE +0x62C, // 0xD3040301

CHB_DDRPHY_BASE +0x614, // 0xD3040301  //27
CHB_DDRPHY_BASE +0x620, // 0xD3040301
CHB_DDRPHY_BASE +0x62C, // 0xD3040301
#endif
};
#endif
...
#define CHA_SRAM_LowFreq_RegVal  0x0012FB00
#define CHB_SRAM_LowFreq_RegVal  0x0012FBD0
#define CHA_SRAM_HighFreq_RegVal 0x0012FCA0
#define CHB_SRAM_HighFreq_RegVal 0x0012FD70
#define PLL_SRAM_LowFreq_RegVal   0x0012FE40
#define PLL_SRAM_HighFreq_RegVal  0x0012FEC0
#define SRAM_pll_setting_num           0x0012FF40
...
void DramcSaveFreqSetting(DRAMC_CTX_T *p)
{
	...
	p->channel = CHANNEL_A;
mcSHOW_DBG_MSG2(("ANDREW, [freq, channel]=[%d, %d]\n", p->frequency, p->channel));
	Dump_Registers(p);
	p->channel = CHANNEL_B;
//mcSHOW_DBG_MSG2(("ANDREW, [freq, channel]=[%d, %d]\n", p->frequency, p->channel));
//    Dump_Registers(p);
	...
	for (Offset = 0; Offset < PLLGRPREG_SIZE; Offset++)
	{
		...
		if (p->frequency == DUAL_FREQ_LOW)
		{
			...
			else if ((Offset >= 24) && (Offset <= 29))
			{
				PLL_LowFreq_RegVal[Offset] = (PLL_LowFreq_RegVal[Offset]&0xfbfffffe)|0x00000001;  	// MEMPLL enable & AUTOK_LOAD=0
			}
			else if(Offset == 6)
			{
				PLL_LowFreq_RegVal[Offset] |= 0x00000002;
			}
			else if(Offset == 7)
			{
				PLL_LowFreq_RegVal[Offset] &= (~0x00000002);
			...
			else if (Offset == (MPLL_PCM+1))
				{
				PLL_LowFreq_RegVal[Offset] &= (~0x80000000);	// MPLL_SDM_PCW_CHG	
				}
                     else if(Offset == 10)		
                     	{				
                     	PLL_LowFreq_RegVal[Offset] &= (~0x00000001);	
                     	}			
                     else if(Offset == 11)			
                     	{				
                     	PLL_LowFreq_RegVal[Offset] |= (0x00000001);
		...
		else
		{
			...
			{
				PLL_HighFreq_RegVal[Offset] = (PLL_HighFreq_RegVal[Offset]&0xfbfffffe)|0x00000001;  	// MEMPLL enable & AUTOK_LOAD=0
			}
			else if(Offset == 6)
			{
				PLL_HighFreq_RegVal[Offset] |= 0x00000002;
			}
			else if(Offset == 7)
			{
				PLL_HighFreq_RegVal[Offset] &= (~0x00000002);
				...
				PLL_HighFreq_RegVal[Offset] &= (~0x80000000);	// MPLL_SDM_PCW_CHG	
                     	...
                     	{				
                     	PLL_HighFreq_RegVal[Offset] &= (~0x00000001);	
                     	}			
                     else if(Offset == 11)			
                     	{				
                     	PLL_HighFreq_RegVal[Offset] |= (0x00000001);
	...
	for (Offset = 0; Offset < (FREQREG_SIZE-1); Offset++)
	{
	    if (p->frequency == DUAL_FREQ_LOW)
	    {
			(*(volatile unsigned int *)(CHA_SRAM_LowFreq_RegVal+(Offset * 4))) = CHA_LowFreq_RegVal[Offset];
			(*(volatile unsigned int *)(CHB_SRAM_LowFreq_RegVal+(Offset * 4))) = CHB_LowFreq_RegVal[Offset];
	    }
	    else
	    {
	    		(*(volatile unsigned int *)(CHA_SRAM_HighFreq_RegVal+(Offset * 4))) = CHA_HighFreq_RegVal[Offset];
			(*(volatile unsigned int *)(CHB_SRAM_HighFreq_RegVal+(Offset * 4))) = CHB_HighFreq_RegVal[Offset];
	    }
       }
		...
		for (Offset = 0; Offset < PLLGRPREG_SIZE; Offset++)
	{
		if (p->frequency == DUAL_FREQ_LOW)
		{
		      (*(volatile unsigned int *)(PLL_SRAM_LowFreq_RegVal+(Offset * 4)))=PLL_LowFreq_RegVal[Offset];
		}
		else
		{
		      (*(volatile unsigned int *)(PLL_SRAM_HighFreq_RegVal+(Offset * 4)))=PLL_HighFreq_RegVal[Offset];
		}
	}

		    (*(volatile unsigned int *)(SRAM_pll_setting_num))  = PLLGRPREG_SIZE;    
                   (*(volatile unsigned int *)(SRAM_freq_setting_num)) = FREQREG_SIZE;
...
void DramcRunTimeConfig(DRAMC_CTX_T *p)
{
	...
	ucstatus |= ucDram_Register_Read(mcSET_DRAMC_REG_ADDR(0x138), &u4value);
        if (p->frequency > 800)
	{
	mcSET_BIT(u4value, 4); //if frequency >1600 , tCKE should >7 clk
    	}
	ucstatus |= ucDram_Register_Write(mcSET_DRAMC_REG_ADDR(0x138), u4value);       
    ...
    if (p->dram_type == TYPE_LPDDR3)
    {
        if(((*(volatile unsigned *)(EMI_APB_BASE+0x00000000)) & 0x1) == 0x1)
        	{//dual channel
