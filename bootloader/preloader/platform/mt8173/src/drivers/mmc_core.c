...
void mmc_dump_ext_csd(struct mmc_card *card)
{
    ...
    printf("[EXT_CSD] Boot area write prot status  : %xh\n", ext_csd[EXT_CSD_BOOT_WP_STATUS]);
...
static void mmc_decode_ext_csd(struct mmc_card *card)
{
	...
	card->host->work_part = ext_csd[EXT_CSD_PART_CFG] & 0x7;
...
#ifdef FEATURE_MMC_BOOT_PROTECT
static uint8 mmc_wp_val[] = {
	MMC_EXT_CSD_BOOT_WP_B1_PWRON,
	MMC_EXT_CSD_BOOT_WP_B1_PERM,
	MMC_EXT_CSD_BOOT_WP_B2_PWRON,
	MMC_EXT_CSD_BOOT_WP_B2_PERM,
	MMC_EXT_CSD_BOOT_WP_B12_PWRON,
	MMC_EXT_CSD_BOOT_WP_B12_PERM,
	MMC_EXT_CSD_BOOT_WP_NONE
};

/* 
 * c - struct mmc_card *card
 * t - enum mmc_boot_wp_type 
 */
#define MMC_SET_BOOT_AREA_WP(c, t) \
	mmc_set_ext_csd(c, EXT_CSD_BOOT_WP, mmc_wp_val[t])
#else
#define MMC_SET_BOOT_AREA_WP(c, t)	0
#endif
...
static void emmc_r_w_compare_test()
{
    ...
    printf("[PL][%s:%d]eMMC simp test start\n", __func__, __LINE__);
    ...
    printf("[PL][%s:%d] finish read\n", __func__, __LINE__);

    /* compara */
    for (i = 0; i < PL_MMC_TEST_SIZE; i++){
        if (g_mmc_buf[i] != 0x5a){
           printf("[PL][%s:%d]mmc simple r/w compare failed(%d is %d, not 0x5a)\n", __func__, __LINE__, i, g_mmc_buf[i]);
           break;
        }
    }
...
void mmc_convert_dma_mode(int id)
{
    struct mmc_host *host;
    BUG_ON(id >= NR_MMC);
    host = &sd_host[id];
	msdc_convert_dma_mode(host);
}
#endif
...
int mmc_init(int id)
{
    ...
    printf("[%s]: msdc%d start in Preloader\n", __func__, id);
    BUG_ON(id >= NR_MMC);

    host = &sd_host[id];
    card = &sd_card[id];
    printf("[%s]: msdc%d, start mmc_init_host() in Preloader\n", __func__, id);
    err = mmc_init_host(host, id);
    if (err == MMC_ERR_NONE){
        printf("[%s]: msdc%d start mmc_init_card() in Preloader\n", __func__, id);
	...
	if (err == MMC_ERR_NONE) {
		err = MMC_SET_BOOT_AREA_WP(card, MMC_BOOT_WP_NONE);
	}

    printf("[%s]: msdc%d init done in Preloader, err=%d\n", __func__, id, err);
    return err;
}
#else
...
int mmc_legacy_init(int verbose)
{
    ...
    if (err == MMC_ERR_NONE && !boot_dev_found) {
        ...
        if (host->boot_type == RAW_BOOT) {
            ...
            boot_dev_found = 1;
            printf("[%s]: msdc%d boot device found\n", __func__, id);
        } else if (host->boot_type == FAT_BOOT) {
            #if (CONFIG_COMMANDS & CFG_CMD_FAT)
            if (0 == fat_register_device(bdev, 1)) {
                boot_dev_found = 1;
                printf("[%s]: msdc%d FAT partition found\n", __func__, id);
            }
            #endif
        }
    }
