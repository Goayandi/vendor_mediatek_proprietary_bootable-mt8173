...
disp_path_handle _display_interface_path_init(disp_lcm_handle *plcm)
{
	...
	memcpy(&(data_config.dispif_config), &(plcm->params->dsi), sizeof(LCM_DSI_PARAMS));
	...
	if (_lcm_count() == 0) {
		...
		if (plcm_name == NULL) {
			...
			if (isLCMFound == false) {
				...
				DISPERR("Falling to default lcm, if no display check default lcm config\n");
				lcm_drv = lcm_driver_list[0];
				isLCMFound = true;
			}
		} else {
...
int disp_lcm_init(disp_lcm_handle *plcm)
{
	...
	if (_is_lcm_inited(plcm)) {
		...
		if (lcm_drv->init) {
			...
			unsigned int time_lcm_init = get_timer(0);;
			dprintf(0, "lcm init takes %d\n", (int)get_timer(time_lcm_init));
#else
		...
		if (LCM_TYPE_DSI == plcm->params->type) {
			...
			unsigned int time_check_lcm_connect = get_timer(0);;
...
#if DISABLE_LCM_INIT || DISABLE_LCM_INIT_V1
			dprintf(0, "check_lcm_connect takes %d\n", (int)get_timer(time_check_lcm_connect));
#endif
		}
