...
kal_bool enter_kpoc_flag = false;
#endif
...
BOOL is_force_boot(void)
{
	if (rtc_boot_check(true)) {
		...
		/* SLAMFOSV-1489-workaround for bypass dkernel mode, need to redefine the booting squence */
		if (g_boot_mode != DKERNEL_BOOT) {
...
BOOL kernel_power_off_charging_detection(void)
{
	...
	if (enter_kpoc_flag == false) {
		dprintf(CRITICAL, "By-pass kernel power-off charging!\n");
		return FALSE;
	}
#endif
