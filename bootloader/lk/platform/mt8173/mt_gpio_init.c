...
void mt_gpio_set_default_ext(void)
{
	...
	mt_set_gpio_mode(GPIOEXT12, GPIOEXT12_MODE);
	mt_set_gpio_mode(GPIOEXT20, GPIOEXT20_MODE);
...
void mt_gpio_set_dsel(u8 md_power)
{
	...
	if (md_power == GPIO_VIO18) {
		...
		GPIO_WR16(&pReg->exmd_ctrl[0].rst, 0x3F5);
		GPIO_WR16(&pReg->exmd_ctrl[0].set, 0xA);
	} else if (md_power == GPIO_VIO28) {
		/*exmd_ctrl[9:4] = b`001100, [3:0] = b`1010*/
		GPIO_WR16(&pReg->exmd_ctrl[0].rst, 0x335);
		GPIO_WR16(&pReg->exmd_ctrl[0].set, 0xCA);
	}
	//GPIOVER("EXMD 0x%x\n", GPIO_RD16(&pReg->exmd_ctrl[0].val));

	/*********** other R/TDSEL **********/
	GPIO_WR16(&pReg->msdc2_ctrl5.set, 0xA);    //msdc1_ctrl4 , bit[3:0] = b`1010
	GPIO_WR16(&pReg->msdc2_ctrl5.rst, 0x5);
	...
	//GPIO_WR16(&pReg->tdsel[3].set, 0x1100);    //tdsel[3], bit[8]= b`1, bit[12]= b`1
	//GPIO_WR16(&pReg->tdsel[5].set, 0x1001);    //tdsel[5], bit[0]= b`1, bit[12]= b`1
	//GPIO_WR16(&pReg->tdsel[6].set, 0x1);       //tdsel[6], bit[0]= b`1
}
/*---------------------------------------------------------------------------*/
void mt_gpio_set_default(void)
{
