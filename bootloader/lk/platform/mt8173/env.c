...
static int write_env_area(char *env_buf)
{
	...
	part = partition_get_index(ENV_PART);
	if (part == INVALID_PTN) {
		return -ENOENT;
	}
	...
	*((int *)env_buf+CFG_ENV_CHECKSUM_OFFSET/4) = checksum;
	...
	len = dev->write(dev, (uchar*)env_buf, start_addr + CFG_ENV_OFFSET, CFG_ENV_SIZE, EMMC_PART_USER);
#else
	len = -1;
#endif
	if (len < 0) {
...
#if defined(MTK_EMMC_SUPPORT) && defined(MTK_NEW_COMBO_EMMC_SUPPORT)
	len = dev->read(dev, start_addr + CFG_ENV_OFFSET, (uchar*)env_buf, CFG_ENV_SIZE, EMMC_PART_USER);
#else
	len = -1;
#endif
	if (len < 0) {
