    ...
    u64 trusty_sec_dram_size, u64 dram_base, u64 dram_size)
{
    ...
    seclib_get_msg_auth_key(teearg->hwuid, 16, teearg->rpmb_key, 32);
    teearg->rpmb_key_size = 32;
...
#if 0
    {
		...
		DBG_MSG("%s teearg->rpmb_key_size=0x%x\n", MOD, teearg->rpmb_key_size);
        int i;
        for(i=0; i<32; i+=4) {
            DBG_MSG("%s teearg->rpmb_key[%d-%d] : 0x%x 0x%x 0x%x 0x%x\n", MOD,
                    i, i+3, teearg->rpmb_key[i], teearg->rpmb_key[i+1],
                    teearg->rpmb_key[i+2], teearg->rpmb_key[i+3]);
        }
    }
#endif
}
