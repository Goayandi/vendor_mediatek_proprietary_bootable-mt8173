...
static u64 tee_secmem_size = 0;
static u64 tee_secmem_start = 0;
...
void tee_get_secmem_size(u32 *size, u32 *atf_log_size)
{
#if CFG_TEE_SUPPORT
...
#endif
    *atf_log_size = ATF_LOG_BUFFER_SIZE;
}
...
#if CFG_MTK_IN_HOUSE_TEE_SUPPORT!=1
extern trustonic_tee_verify(u32 *addr, u32 size, const u8 tee_img_vfy_pubk[]);
#else /* MTK_IN_HOUSE_TEE or ATF */
...
{
    u32 ret_addr = 0;
#if CFG_MTK_IN_HOUSE_TEE_SUPPORT || CFG_GOOGLE_TRUSTY_SUPPORT
...
#if CFG_MTK_IN_HOUSE_TEE_SUPPORT==0 && CFG_GOOGLE_TRUSTY_SUPPORT==0
    ...
    if (CFG_DRAM_ADDR + total_dram_size >= 0x100000000)
        tee_secmem_start = 0x100000000 - tee_secmem_size;
    else
        tee_secmem_start = CFG_DRAM_ADDR + total_dram_size - tee_secmem_size;
#endif
    bootarg.secos_reserved_mem.start = tee_secmem_start - ATF_LOG_BUFFER_SIZE;
    bootarg.secos_reserved_mem.size = tee_secmem_size + ATF_LOG_BUFFER_SIZE;
    ...
    atf_log_buf_start = tee_secmem_start - ATF_LOG_BUFFER_SIZE;
