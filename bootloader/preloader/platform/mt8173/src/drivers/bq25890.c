...
CHARGER_TYPE bq25890_charger_type_detection(void)
{
	...
	bq25890_print("[PL] use BQ25890 charger detection\r\n");
	...
	if (ret_val == 0x7) {
		bq25890_print("charger OTG mode is on during type detection!\n");
		charger_type = CHARGER_UNKNOWN;
		bq25890_set_otg_config(0);
		bq25890_print("charger OTG mode is cleared!\n");
		return charger_type;
	}
	...
	if (ret_val != vbus_state)
		bq25890_print("Update VBUS state from %d to %d!\n", vbus_state, ret_val);
