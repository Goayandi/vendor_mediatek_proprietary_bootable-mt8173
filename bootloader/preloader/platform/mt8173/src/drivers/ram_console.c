...
void ram_console_init(void)
{
    ...
    if (g_rgu_status != 0 && ram_console->sig == RAM_CONSOLE_SIG && ram_console->sz_pl == sizeof(struct reboot_reason_pl)) {
	...
	memcpy(ram_console->off_lpl, ram_console->off_pl, ram_console->sz_pl);
        ...
        print("%s init ram_console: assign sig:0x%x, off_pl:0x%x, sz_pl:0x%x, off_lpl:0x%x\n",
                MOD,
                RAM_CONSOLE_SIG,
                sizeof(struct ram_console_buffer),
                sizeof(struct reboot_reason_pl),
                sizeof(struct ram_console_buffer) + ALIGN(sizeof(struct reboot_reason_pl), 64));
	...
	ram_console->off_lpl = ram_console->off_pl + ALIGN(ram_console->sz_pl, 64);
        print("%s init ram_console: after sig(0x%x):0x%x, off_pl(0x%x):0x%x, sz_pl(0x%x):0x%x, off_lpl(0x%x):0x%x\n",
                MOD,
                &(ram_console->sig), ram_console->sig,
                &(ram_console->off_pl), ram_console->off_pl,
                &(ram_console->sz_pl), ram_console->sz_pl,
                &(ram_console->off_lpl), ram_console->off_lpl);
    }
...
void ram_console_reboot_reason_save(u8 rgu_status)
{
	...
	print("%s wdt status (0x%x)=0x%x\n", MOD,
	      &rr_pl->wdt_status, rgu_status);
        print("%s %s: after sig(0x%x):0x%x, off_pl(0x%x):0x%x, sz_pl(0x%x):0x%x, off_lpl(0x%x):0x%x\n",
                MOD, __func__, 
                &(ram_console->sig), ram_console->sig,
                &(ram_console->off_pl), ram_console->off_pl,
                &(ram_console->sz_pl), ram_console->sz_pl,
