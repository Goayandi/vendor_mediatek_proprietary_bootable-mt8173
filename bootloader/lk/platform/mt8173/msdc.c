...
static gpd_t msdc_gpd_pool[MSDC_MAX_NUM][MAX_GPD_POOL_SZ] __attribute__ ((aligned(64)));
...
static void msdc_top_reset(struct mmc_host *host)
{
	...
	for (i = 0; i < TOP_RESET_BACKUP_REG_NUM; i++) {
		...
		if (top_reset_backup_reg_list[i].restore_func) {
			...
			if (err) {
				dprintf(CRITICAL, "failed to backup reg[0x%x][0x%x], expected value[0x%x], actual value[0x%x] err=0x%x",
	...
	dprintf(CRITICAL, "[%s]: top reset\n",__func__);
	MSDC_SET_FIELD(0xf0003000, (0x1 << 19), 0x1);
	mdelay(1);
	MSDC_SET_FIELD(0xf0003000, (0x1 << 19), 0x0);
	emmc_top_reset = 1;

	dprintf(CRITICAL, "[%s]: restore\n",__func__);
	for (i = 0; i < TOP_RESET_BACKUP_REG_NUM; i++) {
		...
		if (top_reset_backup_reg_list[i].restore_func) {
			...
			if (err) {
				dprintf(CRITICAL, "failed to restore reg[0x%x][0x%x], expected value[0x%x], actual value[0x%x] err=0x%x\n",
				        top_reset_backup_reg_list[i].addr, top_reset_backup_reg_list[i].mask, top_reset_backup_reg_list[i].value, MSDC_READ32(top_reset_backup_reg_list[i].addr), err);
			}
		}
	}
...
void msdc_dump_padctl(struct mmc_host *host, u32 pin)
{
	switch (pin) {
		case GPIO_CLK_CTRL:
			dprintf(CRITICAL, "  CLK_PAD_CTRL        = 0x%x\n", MSDC_READ32(MSDC0_GPIO_CLK_BASE));
			break;
		case GPIO_CMD_CTRL:
			dprintf(CRITICAL, "  CMD_PAD_CTRL        = 0x%x\n", MSDC_READ32(MSDC0_GPIO_CMD_BASE));
			break;
		case GPIO_DAT_CTRL:
			dprintf(CRITICAL, "  DAT_PAD_CTRL        = 0x%x\n", MSDC_READ32(MSDC0_GPIO_DAT_BASE));
			break;
		case GPIO_RST_CTRL:
			dprintf(CRITICAL, "  RST_PAD_CTRL        = 0x%x\n", MSDC_READ32(MSDC0_GPIO_RST_BASE));
			break;
		case GPIO_DS_CTRL:
			dprintf(CRITICAL, "  DS_PAD_CTRL         = 0x%x\n", MSDC_READ32(MSDC0_GPIO_DS_BASE));
			break;
		case GPIO_MODE_CTRL:
			dprintf(CRITICAL, "  PAD_MODE_CTRL         = 0x%x\n", MSDC_READ32(MSDC0_GPIO_MODE0_BASE));
...
static void msdc_dump_dbg_register(struct mmc_host *host)
{
	...
	for (i = 0; i < 26; i++) {
		...
		dprintf(CRITICAL, "SW_DBG_SEL: write reg[%x] to 0x%x\n", OFFSET_MSDC_DBG_SEL, i);
		dprintf(CRITICAL, "SW_DBG_OUT: read  reg[%x] to 0x%x\n", OFFSET_MSDC_DBG_OUT, MSDC_READ32(MSDC_DBG_OUT));
	}
...
static void msdc_dump_clock_sts(struct mmc_host *host)
{
	...
	dprintf(CRITICAL, " pll[0x1020925c][bit0~1 should be 0x1]=0x%x\n", MSDC_READ32(0x1020925c));
	dprintf(CRITICAL, " pll[0x10209250][bit0 should be 0x1]=0x%x\n", MSDC_READ32(0x10209250));
	dprintf(CRITICAL, " mux[0x10000070][should be 0x02060301]=0x%x\n", MSDC_READ32(0x10000070));
	dprintf(CRITICAL, " clk[0x10003018][bit13 for msdc0, bit14 for msdc1]=0x%x\n", MSDC_READ32(0x10003018));
#endif
}
...
static void msdc_dump_ldo_sts(struct mmc_host *host)
{
	...
	dprintf(CRITICAL, " VEMC_EN[0x552][bit10 should be 1]=0x%d, VEMC_VOL[0x56a][bit6 should be 1]=0x%x\n", ldo_en, ldo_vol);
#endif
}
...
static void msdc_dump_info(struct mmc_host *host)
{
	dprintf(CRITICAL, "[%s]: error happend,here\n", __func__);
}
#endif
...
void msdc_set_card_pwr(int on)
{
	...
	MSDC_WRITE32(PWR_GPIO_EO, (PWR_MASK_VOL_18 | PWR_MASK_EN)); // | PWR_MASK_L4));

	if (on) {
		//dprintf(INFO, "[%s]: start set gpio, on\n", __func__);
		MSDC_WRITE32(PWR_GPIO, (PWR_MASK_VOL_18 | PWR_MASK_EN)); // | PWR_MASK_L4));
	} else {
		//dprintf(INFO, "[%s]: start set gpio, off\n", __func__);
		MSDC_WRITE32(PWR_GPIO, 0x0);
	}
	//dprintf(INFO, "[%s]: before delay(3)\n", __func__);
	mdelay(3);
	dprintf(INFO, "[%s]: finish, on=%d, PWR_GPIO_EO=0x%x, PWR_GPIO=0x%x\n", __func__, on, MSDC_READ32(PWR_GPIO_EO), MSDC_READ32(PWR_GPIO));
}
...
int msdc_abort_handler(struct mmc_host *host, int abort_card)
{
	...
	while (state != 4) { // until status to "tran"
		...
		dprintf(CRITICAL, "check card state<%d>\n", state);
		if (state == 5 || state == 6) {
	...
	if (opcode == MMC_CMD_SEND_STATUS) {
		if (SDC_IS_CMD_BUSY()) {
			...
			if (timeout == 0) {
				...
				dprintf(CRITICAL, "[SD%d] CMD(%d): SDC_IS_CMD_BUSY timeout\n",
		...
		if (SDC_IS_BUSY()) {
			...
			if (timeout == 0) {
				error = MMC_ERR_TIMEOUT;
				dprintf(CRITICAL, "[SD%d] CMD(%d): SDC_IS_BUSY timeout\n",
				        host->id, (opcode & ~(SD_CMD_BIT | SD_CMD_APP_BIT)));
				goto end;
			}
		}
	...
	if (err == MMC_ERR_BADCRC) {
		...
		if (tmp & 0x1800) {
			if (msdc_abort_handler(host, 1))
				dprintf(CRITICAL, "[SD%d] abort failed\n",host->id);
		}
...
int msdc_dma_wait_done(struct mmc_host *host, u32 timeout)
{
	...
	do {
		...
		if (status == 0 || status & MSDC_INT_DATTMO) {
			dprintf(CRITICAL, "[SD%d] DMA DAT timeout(%xh)\n", host->id, status);
			error = MMC_ERR_TIMEOUT;
			goto end;
		} else if (status & MSDC_INT_DATCRCERR) {
			dprintf(CRITICAL, "[SD%d] DMA DAT CRC error(%xh)\n", host->id, status);
			error = MMC_ERR_BADCRC;
			g_crc_count++;
			goto end;
		} else if (status & MSDC_INT_CMDTMO) {
			dprintf(CRITICAL, "[SD%d] DMA CMD timeout(%xh)\n", host->id, status);
			error = MMC_ERR_TIMEOUT;
			goto end;
		} else if (status & MSDC_INT_RSPCRCERR) {
			dprintf(CRITICAL, "[SD%d] DMA CMD CRC error(%xh)\n", host->id, status);
			error = MMC_ERR_BADCRC;
			g_crc_count++;
			goto end;
		} else if (status & MSDC_INT_ACMDTMO) {
			dprintf(CRITICAL, "[SD%d] DMA ACMD timeout(%xh)\n", host->id, status);
			error = MMC_ERR_TIMEOUT;
			goto end;
		} else if (status & MSDC_INT_ACMDCRCERR) {
			dprintf(CRITICAL, "[SD%d] DMA ACMD CRC error(%xh)\n", host->id, status);
			error = MMC_ERR_BADCRC;
			g_crc_count++;
			goto end;
		}
	...
	do {
		...
		if (status & MSDC_INT_GPDCSERR) {
			dprintf(CRITICAL, "[SD%d] GPD checksum error\n", host->id);
			error = MMC_ERR_BADCRC;
			g_crc_count++;
			break;
		} else if (status & MSDC_INT_BDCSERR) {
			dprintf(CRITICAL, "[SD%d] BD checksum error\n", host->id);
			error = MMC_ERR_BADCRC;
			g_crc_count++;
			break;
...
int msdc_dma_transfer(struct mmc_host *host, struct mmc_command *cmd, u8 *buf, u32 nblks)
{
	...
	if (derr != MMC_ERR_NONE) {
		dprintf(CRITICAL, "[SD%d] <CMD%d> DMA data error (%d)\n", host->id, cmd->opcode, derr);
		if (msdc_abort_handler(host, 1))
			dprintf(CRITICAL, "[SD%d] abort failed\n",host->id);
	}
...
int msdc_pio_read_word(struct mmc_host *host, u32 *ptr, u32 size)
{
	...
	while (1) {
		...
		if (status & MSDC_INT_DATCRCERR) {
			dprintf(CRITICAL, "[SD%d] DAT CRC error (0x%x), Left:%d/%d bytes, RXFIFO:%d\n",
		...
		if ((size >=  MSDC_FIFO_THD) && (MSDC_RXFIFOCNT() >= MSDC_FIFO_THD)) {
			...
			do {
				...
				dprintf(CRITICAL, "0x%x ", MSDC_FIFO_READ32());
#else
			...
			while (size) {
				if (size > 3) {
					...
					*ptr++ = MSDC_FIFO_READ32();
					...
					while (size --) {
#ifdef MTK_MSDC_DUMP_FIFO
						dprintf(CRITICAL, "0x%x ", MSDC_FIFO_READ8());
#else
...
int msdc_pio_write_word(struct mmc_host *host, u32 *ptr, u32 size)
{
	...
	while (1) {
		...
		if (status & MSDC_INT_DATCRCERR) {
			dprintf(CRITICAL, "[SD%d] DAT CRC error (0x%x), Left DAT: %d bytes\n",
			        host->id, status, size);
			err = MMC_ERR_BADCRC;
			g_crc_count++;
			break;
		} else if (status & MSDC_INT_DATTMO) {
			dprintf(CRITICAL, "[SD%d] DAT TMO error (0x%x), Left DAT: %d bytes\n",
			        host->id, status, size);
			err = MMC_ERR_TIMEOUT;
			break;
		} else if (status & MSDC_INT_XFER_COMPL) {
...
int msdc_pio_bread(struct mmc_host *host, u8 *dst, u32 src, u32 nblks)
{
	...
	if (err != MMC_ERR_NONE) {
		if (derr != MMC_ERR_NONE) {
			dprintf(CRITICAL, "[SD%d] Read data error (%d)\n", host->id, derr);
			if (msdc_abort_handler(host, 1))
				dprintf(CRITICAL, "[SD%d] abort failed\n",host->id);
		} else {
			dprintf(CRITICAL, "[SD%d] Read error (%d)\n", host->id, err);
		}
	}
...
int msdc_pio_bwrite(struct mmc_host *host, u32 dst, u8 *src, u32 nblks)
{
	...
	if (err != MMC_ERR_NONE) {
		if (derr != MMC_ERR_NONE) {
			dprintf(CRITICAL, "[SD%d] Write data error (%d)\n", host->id, derr);
			if (msdc_abort_handler(host, 1))
				dprintf(CRITICAL, "[SD%d] abort failed\n",host->id);
		} else {
			dprintf(CRITICAL, "[SD%d] Write error (%d)\n", host->id, err);
		}
	}
	return (derr == MMC_ERR_NONE) ? err : derr;
}
...
void msdc_config_clock(struct mmc_host *host, int ddr, u32 hz)
{
	...
	//dprintf(DEBUG,"MSDC_CFG_CKMOD|MSDC_CFG_CKDIV<0x%x>\n",temp);
...
#ifdef MTK_MSDC_BRINGUP_DEBUG
	dprintf(CRITICAL, "[%s]: msdc%d, pll[0xf020925c][bit0~1=0x1]=0x%x, pll[0xf0209250][bit0=0x1]=0x%x, mux[0xf0000070][bit15,23,31=0x1]=0x%x, clk[0xf0003018][bit13=msdc0, bit14=msdc1]=0x%x\n",
			...
			do {
				for (rsmpl = 0; rsmpl < 2; rsmpl++) {
					...
					/* for debugging */
					{
						...
						//dprintf(INFO, "[SD%d] <TUNE_CMD><%d><%s> MSDC_CKGEN_RX_SDCLKO_SEL=%xh\n",
					...
					/*if ((result == MMC_ERR_TIMEOUT || result == MMC_ERR_BADCRC)&&  cmd->opcode == MMC_CMD_STOP_TRANSMISSION){
					    dprintf(CRITICAL, "[SD%d]TUNE_CMD12--failed ignore\n",host->id);
					    result = MMC_ERR_NONE;
					    goto done;
					}*/
					if (cmd->opcode == MMC_CMD_READ_MULTIPLE_BLOCK || cmd->opcode == MMC_CMD_WRITE_MULTIPLE_BLOCK || cmd->opcode == MMC_CMD_READ_SINGLE_BLOCK ||cmd->opcode == MMC_CMD_WRITE_BLOCK)
			...
			do {
				for (dsmpl = 0; dsmpl < 2; dsmpl++) {
...
#if MSDC_TUNE_LOG
					{
						...
						//dprintf(INFO, "[SD%d] <TUNE_BREAD_%d><%s> MSDC_CKGEN_RX_SDCLKO_SEL=%xh\n",
						//host->id, times, (result == MMC_ERR_NONE && dcrc == 0) ?
						//"PASS" : "FAIL", t_cksel);
...
int msdc_tune_read(struct mmc_host *host)
{
	...
	u32 cur_dat0 = 0, cur_dat1 = 0, cur_dat2 = 0, cur_dat3 = 0, cur_dat4 = 0, cur_dat5 = 0, cur_dat6 = 0, cur_dat7 = 0;
			...
			do {
				for (dsmpl = 0; dsmpl < 2; dsmpl++) {
...
#if MSDC_TUNE_LOG
					{
						...
						dprintf(CRITICAL, "[SD%d] <TUNE_BWRITE_%d><%s> DSPL=%d, DATWRDLY=%d\n",
						        host->id, times, result == MMC_ERR_NONE ? "PASS" : "FAIL",
						        t_dspl, t_wrrdly);
						dprintf(CRITICAL, "[SD%d] <TUNE_BWRITE_%d><%s> MSDC_DAT_RDDLY0=%xh\n",
						        host->id, times, (result == MMC_ERR_NONE) ? "PASS" : "FAIL",
						        MSDC_READ32(MSDC_DAT_RDDLY0));
						/* dprintf(CRITICAL, "[SD%d] <TUNE_BWRITE_%d><%s> DDR50CKD=%xh\n",
						        host->id, times, (result == MMC_ERR_NONE) ? "PASS" : "FAIL",
						        t_ddrdly);*/
						dprintf(CRITICAL, "[SD%d] <TUNE_BWRITE_%d><%s> MSDC_PB1_WRDAT_CRCS_TA_CNTR=%xh\n",
						        host->id, times, (result == MMC_ERR_NONE) ? "PASS" : "FAIL",
						        t_d_cntr);
						//dprintf(CRITICAL,"[SD%d] <TUNE_BWRITE_%d><%s> MSDC_CKGEN_RX_SDCLKO_SEL=%xh\n",
						//host->id, times, (result == MMC_ERR_NONE) ? "PASS" : "FAIL",
						//t_cksel);
...
int msdc_emmc_boot_start(struct mmc_host *host, u32 hz, int ddr, int mode, int ackdis)
{
	...
	dprintf(CRITICAL,"[SD%d] EMMC BOOT ACK timeout: %d ms (clkcnt: %d)\n", host->id,
	        (acktmo * 256) / (host->sclk / 1000), acktmo);
	dprintf(CRITICAL, "[SD%d] EMMC BOOT DAT timeout: %d ms (clkcnt: %d)\n", host->id,
	...
	if (!ackdis) {
		do {
			...
			if (sts & EMMC_STS_BOOTACKRCV) {
				...
				dprintf(CRITICAL, "[SD%d] EMMC_STS(%x): boot up ack error\n", host->id, sts);
				err = MMC_ERR_BADCRC;
				goto out;
			} else if (sts & EMMC_STS_BOOTACKTMO) {
				dprintf(CRITICAL, "[SD%d] EMMC_STS(%x): boot up ack timeout\n", host->id, sts);
				err = MMC_ERR_TIMEOUT;
				goto out;
			} else if (sts & EMMC_STS_BOOTUPSTATE) {
				dprintf(INFO,"[SD%d] EMMC_STS(%x): boot up mode state\n", host->id, sts);
			} else {
				dprintf(CRITICAL, "[SD%d] EMMC_STS(%x): boot up unexpected\n", host->id, sts);
			}
		} while (1);
	}
	...
	do {
		...
		if (sts & EMMC_STS_BOOTDATRCV) {
			dprintf(CRITICAL, "[SD%d] EMMC_STS(%x): boot dat received\n", host->id, sts);
			break;
		}
...
out:
	if (err) {
		dprintf(CRITICAL, "[SD%d] EMMC_BOOT: read boot code fail(%d), FIFOCNT=%d\n",
...
int msdc_init(struct mmc_host *host, int id)
{
	...
	dprintf(CRITICAL, "[%s]: msdc%d Host controller intialization start in LK\n", __func__, id);
	...
	dprintf(INFO, "set up GPIO for MSDC\n");
#endif
	...
	dprintf(CRITICAL, "[%s]: msdc%d Host controller intialization done in LK\n", __func__, id);
	return 0;
}
