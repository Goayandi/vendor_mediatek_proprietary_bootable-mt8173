...
int get_partition_info_by_name(const u8 *part_name, u64 *start_lba,
		u64 *size_in_blk)
{
	int err = -1;
	int i, j;
	...
	u32 entries_num;
	u32 entry_size;
	u8 name_len = strlen(part_name);

	/* Read GPT header */
	err = read_data(buf, EMMC_PART_USER, GPT_HEADER_LBA,
			BLOCK_SIZE);
	if (err) {
		efi_err("%s read lba=%llx, err(%d)\n",
			TAG, GPT_HEADER_LBA, err);
		return err;
	}

	/* Read number of partition entries in array */
	/* Read Size of a single partition entry */
	entries_num = *((u32*)&buf[GPT_ENTRIES_NUM_OFFSET]);
	entry_size = *((u32*)&buf[GPT_ENTRY_SIZE_OFFSET]);
	/* efi_err("%s => entries number:%d, size:%d, name_len=%d\n",
			__func__, entries_num, entry_size, name_len); */

	/* Read starting LBA of array of partition entries
	   (always 2 in primary copy) */
	for (i = 0; i < entries_num >> 2; i++) {
		err = read_data(buf, EMMC_PART_USER,
				GPT_ENTRIES_LBA + i, BLOCK_SIZE);
		if (err) {
			efi_err("%s read lba=%llx, err(%d)\n",
				TAG, GPT_ENTRIES_LBA, err);
			return err;
		} else {
			u64 last_lba;
			u8 *ptr = buf;
			...
			for (j = 0; j < 4; j++) {
				...
				for (k = 0; k < name_len; k++) {
					/* efi_err("%s => part_name=%c%c%c%c%c\n", __func__,
							ptr[GPT_ENTRY_NAME_OFFSET],
							ptr[GPT_ENTRY_NAME_OFFSET + 2],
							ptr[GPT_ENTRY_NAME_OFFSET + 4],
							ptr[GPT_ENTRY_NAME_OFFSET + 6],
							ptr[GPT_ENTRY_NAME_OFFSET + 8]); */
					if (part_name[k] != ptr[GPT_ENTRY_NAME_OFFSET + (k << 1)]) {
						match = false;
						break;
					}
				}
				...
				*start_lba = *((u64*)&ptr[GPT_ENTRY_FST_LBA_OFFSET]);
				last_lba = *((u64*)&ptr[GPT_ENTRY_LST_LBA_OFFSET]);
				*size_in_blk = last_lba - *start_lba + 1;
				efi_err("%s => Part %s: start lba: 0x%llx size: 0x%llx\n",
					__func__, part_name, *start_lba, *size_in_blk);
	...
	efi_err("%s cannot find partition: %s\n",
		TAG, part_name);
