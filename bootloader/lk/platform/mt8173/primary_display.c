...
static DISP_MODULE_ENUM _get_dst_module_by_lcm(disp_lcm_handle *plcm)
{
	...
	DISPMSG("plcm is DISP_MODULE_DPI0\n");
	return DISP_MODULE_DPI0;
#elif 0//HDMI_SUB_PATH_LK
	DISPMSG("plcm is DISP_MODULE_DSI0\n");
...
const char* primary_display_get_lcm_name(void)
{
	...
	plcm = disp_lcm_probe(NULL, LCM_INTERFACE_NOTDEFINED);
	//lcm_drv = plcm->drv;

	return disp_lcm_get_name(plcm);
#else
...
int primary_display_init(char *lcm_name)
{
	...
	//mtk_wdt_disable();
	//unsigned int resolutionmode = 0xb; // 1080p60hz
	unsigned int resolutionmode = BOX_FIX_RES; // 720p60hz
	//unsigned int resolutionmode = 0x17; // HDMI_VIDEO_2160P_30HZ
...
#if HDMI_MAIN_PATH_LK
	hdmi_checkedid(0);
	resolutionmode = vOutputResolution();
	set_hdmi_tmds_driver(resolutionmode, deepcolor, colorspace, audiofs,  chnumber,  audiotype);
#endif
	dpmgr_init();
#ifndef DDP_LK_BOOT
	disp_mutex_init(&(pgc->lock));
#endif
	_primary_path_lock();
...
#if HDMI_MAIN_PATH_LK //|| HDMI_SUB_PATH_LK
	data_config.dst_w = HDMI_DISP_WIDTH_LK;
	data_config.dst_h = HDMI_DISP_HEIGHT_LK;
#else
	//memcpy(&(data_config.dispif_config), &(lcm_param->dsi), sizeof(LCM_DSI_PARAMS));
...
extern void disp_path_clock_off(char *str);
...
{
#if HDMI_MAIN_PATH_LK || (HDMI_SUB_PATH_LK && DISABLE_LCM_INIT)
	return HDMI_DISP_WIDTH_LK;
#else
	if (pgc->plcm == NULL) {
...
int primary_display_trigger(int blocking)
{
	int ret = 0;
	DISPFUNC();
	//ret = disp_lcm_init(pgc->plcm);	//wisky-lxh@20170525,fix gost screen w100
	_primary_path_lock();
	if (pgc->mode == DIRECT_LINK_MODE || pgc->mode == SINGLE_LAYER_MODE) {
		if (dpmgr_path_is_busy(pgc->dpmgr_handle)) {
			if (primary_display_is_video_mode()) {
				_primary_path_unlock();
				DISPERR("lcm is busy && video mode\n");
				return 0;
			}
...
int primary_display_config_input(disp_input_config* input)
{
	...
	//int layer =0;
	DISPFUNC();
	printf("primary_display_config_input\n");
	disp_ddp_path_config data_config;
	/* memset((void*)&data_config, 0, sizeof(disp_ddp_path_config)); */
	...
	if (pgc->mode == DIRECT_LINK_MODE || pgc->mode == DECOUPLE_MODE) {
		...
		ret = _convert_disp_input_to_ovl((OVL_CONFIG_STRUCT *)&data_config.ovl_config[input->layer], input);
