...
#if CFG_RTC_REG_EMERGENCY_DL_SUPPORT
void rtc_exit_emergency_dl_mode(void)
{
    u16 spar0;
    ...
    flag to indicate if entering emergency download mode
    it is set by rtc_boot_check() function
    bit 9: set to 1 by "adb reboot emergency", indicate the sytem should enter brom dl mode
    */
    if(spar0&0x0200)
    {
        emergency_dl_flag = false;
        RTC_Write(RTC_SPAR0, spar0 & (~0x0200));
        Write_trigger();
        print("[RTC] clear emergency dl mode flag in rtc register\n");
    }
...
bool rtc_boot_check(void)
{
	...
	rtc_save_2sec_stat();
        ...
        if(spar0&0x0200)
        {
            emergency_dl_flag = true;
/*          RTC_Write(RTC_SPAR0, spar0 & (~0x0200));
                        Write_trigger();
*/
                        print("[RTC] emergency dl mode flag in rtc register is detected\n");
        }
#endif /* CFG_RTC_REG_EMERGENCY_DL_SUPPORT */
