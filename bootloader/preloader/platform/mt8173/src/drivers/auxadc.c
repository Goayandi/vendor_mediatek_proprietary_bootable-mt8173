...
#define auxadc_print(fmt, args...) printf(fmt, ##args);
...
int IMM_GetOneChannelValue(int dwChannel, int data[4], int* rawdata)
{
  ...
  // mutex_lock(&mutex_get_cali_value);
   ...
   {
	    //auxadc_print("hwEnableClock AUXADC !!!.");
	    if(enable_clock(MT65XX_PDN_PERI_AUXADC,"AUXADC"))
   ...
   *(volatile unsigned int *)AUXADC_CLOCK = *(volatile unsigned int *)AUXADC_CLOCK | (1 << 28);
   auxadc_print("[adc_api]: AUXADC_CLOCK = %x.\n", *(volatile unsigned int *)AUXADC_CLOCK);

   *(volatile unsigned short *)AUXADC_MISC = *(volatile unsigned short *)AUXADC_MISC | (1 << 14);
   auxadc_print("[adc_api]: AUXADC_MISC = %x.\n", *(volatile unsigned short *)AUXADC_MISC);

   //if(dwChannel == PAD_AUX_XP)mt_auxadc_disable_penirq();	
   //step1 check con3 if auxadc is busy
   while ((*(volatile unsigned short *)AUXADC_CON2) & 0x01) 
   {
       auxadc_print("[adc_api]: wait for module idle\n");
       udelay(1000);
	   idle_count++;
	   if(idle_count>30)
	   {
	      //wait for idle time out
	      auxadc_print("[adc_api]: wait for aux/adc idle time out\n");
	      return -1;
	   }
   } 
   // step2 clear bit
   if(0 == adc_auto_set)
   {
	   ...
	   *(volatile unsigned short *)AUXADC_CON1 = *(volatile unsigned short *)AUXADC_CON1 & (~(1 << dwChannel));
	   auxadc_print("[adc_api0]: AUXADC_CON1 = %x.\n", *(volatile unsigned short *)AUXADC_CON1);
   }
   ...
   {
       auxadc_print("[adc_api]: wait for channel[%d] ready bit clear\n",dwChannel);
       udelay(10000);
	   data_ready_count++;
	   if(data_ready_count>30)
	   {
	      //wait for idle time out
	      auxadc_print("[adc_api]: wait for channel[%d] ready bit clear time out\n",dwChannel);
	      return -2;
	   }
   }
   ...
   {
       auxadc_print("[adc_api]: wait for channel[%d] ready bit ==1\n",dwChannel);
       udelay(100000);
	 data_ready_count++;
	 if(data_ready_count>30)
	 {
	      //wait for idle time out
	      auxadc_print("[adc_api]: wait for channel[%d] data ready time out\n",dwChannel);
	      return -3;
	 }
   }
   ////step6 read data
...
int IMM_GetOneChannelValue_Cali(int Channel, int*voltage)
{
     ...
     if(ret)
     {
	   ...
	   if(ret)
	   {
	        auxadc_print("[adc_api]:IMM_GetOneChannelValue_Cali  get raw value error %d \n",ret);
		  return -1;
	   }
     }
     ...
     *voltage = rawvalue*1500000 / 4096;
      ...
      //auxadc_print("[adc_api]:IMM_GetOneChannelValue_Cali  voltage= %d uv \n",*voltage);
...
static int IMM_auxadc_get_evrage_data(int times, int Channel)
{
	...
	while (i--)
	{
		...
		ret += ret_temp;
		auxadc_print("[auxadc_get_data(channel%d)]: ret_temp=%d, ret = ret_value.\n",Channel,ret_temp,ret_value);        
	}
...
int auxadc_test(void ) 
{
   ...
   //int Voltiage_cali =0;
      ...
      for (i = 0; i < 16; i++) 
      {
        //auxadc_print("[adc_driver]: i=%d\n",i);
       
		res = IMM_GetOneChannelValue(i,data,&rawdata);
		if(res < 0)
		{ 
			   auxadc_print("[adc_lk]: get data error\n");
			   break;
			   
		}
		else
		{
		       auxadc_print("[adc_lk]: channel[%d]raw =%d\n",i,rawdata);
		       //auxadc_print("[adc_lk]: channel[%d]=%d.%.02d \n",i,data[0],data[1]);
		...
		if(res < 0)
		{ 
			   auxadc_print("[adc_driver]: get cali voltage error\n");
			   break;
			   
		}
		else
		{
		       auxadc_print("[adc_driver]: channel[%d] cali_voltage =%d\n",i,Voltiage_cali);
