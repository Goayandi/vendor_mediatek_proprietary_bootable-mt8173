...
do { \
    if (ret == 0) \
        printf("bq24297 read/write error.\n"); \
} while(0)
...
static void bq_set_charging_current(kal_int32 cc_value)
{
	if (cc_value > 2484 || cc_value < 500) {
		bq24297_print("invalid current selected (%d), use 500mA \r\n", cc_value);
		bq24297_set_ichg(0);
	} else
		bq24297_set_ichg((cc_value-512)/64);
}
...
static void bq_set_reg_voltage(kal_int32 cv)
{
	if (cv == 4200)
		...
		bq24297_set_vreg(0x1F); //VREG 4.0V
	else
		bq24297_set_vreg(0x2C); //VREG 4.208V
}
...
void probe_bq24297(void)
{
	kal_uint32 part_num;

	part_num = bq24297_get_pn();
	if (part_num == 0x3) {
		charger.charger_type_detection = bq24297_charger_type_detection;
		register_charger_control();
		bq24297_print("find BQ24297 device and register charger control.\n");
	} else if (part_num == 0x1) {
		register_charger_control();
		bq24297_print("find BQ24296 device and register charger control.\n");
	} else
		bq24297_print("no BQ24297/BQ24296 device is found.\n");
}
