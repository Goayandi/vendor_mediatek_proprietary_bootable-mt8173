...
{
#if !CFG_FPGA_PLATFORM
	if ((CFG_UART_META == UART2) || (CFG_UART_LOG == UART2)) {
	...
	if ((CFG_UART_META == UART3) || (CFG_UART_LOG == UART3)) {
	...
	if ((CFG_UART_META == UART4) || (CFG_UART_LOG == UART4)) {
    ...
    UART_CLR_BITS(1 << 19, AP_PERI_GLOBALCON_PDN0); /* Power on UART1 */
...
{
    while(!(UART_READ32 (UART_LSR(UART1)) & UART_LSR_THRE))
    {
    }

    if (c == '\n')
        UART_WRITE32((unsigned int) '\r', UART_THR(UART1));

    UART_WRITE32((unsigned int) c, UART_THR(UART1));
}
#endif
