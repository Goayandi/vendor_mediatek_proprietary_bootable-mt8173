...
extern int get_partition_info_by_name(const u8 *part_name, u64 *start_lba,
										u64 *size_in_blk);
int part_read_dramc(u32 offset_in_blk, u32 size_in_blk, u8 *buf)
{
	int err = -1;
	u64 start_lba, size;
	blkdev_t *dev;

	err = get_partition_info_by_name("DRAMC", &start_lba, &size);
	if (err) {
		print("%s get DRAMC partition info failed. (err %d)\n",
				__func__, err);
		return err;
	}

	if ((offset_in_blk >= size) ||
			(offset_in_blk + size_in_blk > size)) {
	...
	if (!dev) {
		print("%s read data, err(no dev)\n", __func__);
		return -1;
	}

	err = dev->bread(dev, start_lba + offset_in_blk, size_in_blk, buf, EMMC_PART_USER);
	if (err) {
		print("%s read data, err(%d)\n", __func__, err);
		return err;
	}
...
int part_write_dramc(u32 offset_in_blk, u32 size_in_blk, u8 *buf)
{
	int err = -1;
	u64 start_lba, size;
	blkdev_t *dev;
	...
	dev = blkdev_get(CFG_BOOT_DEV);
	if (!dev) {
		print("%s write data, err(no dev)\n", __func__);
		return -1;
	}

	err = dev->bwrite(dev, start_lba + offset_in_blk, size_in_blk, buf, EMMC_PART_USER);
	if (err) {
		print("%s write data, err(%d)\n", __func__, err);
		return err;
	}
...
int read_fastlogo_info(void *data)
{
    u8 res = 0;
	...
	bdev = blkdev_get(CFG_BOOT_DEV);
    blksz = bdev->blksz;
    /* only read the last  block at logo */
    if (!data) {
                printf("[%s] %s Invalied data!\n", MOD, __func__);
		return -1;
    }

    part_t *fastlogo_part = part_get(PART_LOGO);
    if (!fastlogo_part) {
		printf("[%s] Invalied partition name!\n", MOD, __func__);
		return -1;
    }

#ifdef MTK_EMMC_SUPPORT
	start_blk = fastlogo_part->start_sect;
#else
	start_blk = fastlogo_part->startblk;
#endif

	
	res =  blkdev_read(bdev, start_blk*blksz + PART_SIZE_LOGO - blksz, blksz, data, fastlogo_part->part_id); 

        printf("[%s] %s start_blk %d blksz %d  PART_SIZE_LOGO %d res %d!\n", 
        MOD, __func__, start_blk, blksz, PART_SIZE_LOGO, res);
#endif
