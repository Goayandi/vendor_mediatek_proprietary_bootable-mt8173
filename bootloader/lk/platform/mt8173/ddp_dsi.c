...
#define DSI_OUTREG32(cmdq, addr, val) \
    {\
        ...
        {\
            UINT32 *tmp1 = (UINT32 *)(addr); \
            UINT32 tmp2 = (UINT32)(val); \
            mt65xx_reg_sync_writel(tmp2, tmp1);\
        }}
...
#define DSI_OUTREGBIT(cmdq, TYPE,REG,bit,value)  \
    ...
    {\
        do {    \
            UINT32 *addr = (UINT32 *)(&REG);     \
            UINT32 *valu;   \
            TYPE r = *((TYPE*)&INREG32(addr));    \
            r.bit = value;    \
            valu = (UINT32 *)(&r);\
            DSI_OUTREG32(cmdq, addr, *valu);      \
            } while (0);\
    }}
...
#define MIPITX_OUTREGBIT(TYPE,REG,bit,value)  \
    {\
        do {    \
            UINT32 *tmp1 = (UINT32 *)(&REG);     \
            UINT32 *tmp2;   \
            TYPE r = *((TYPE*)&INREG32(tmp1));    \
                        r.bit = value;    \
            tmp2 = (UINT32 *)(&r);\
            mt65xx_reg_sync_writel(*tmp2, tmp1); \
            } while (0);\
...
DSI_STATUS DSI_DisableClk(DISP_MODULE_ENUM module, void* cmdq)
{
	int i = 0;
...
void DSI_PHY_TIMCONFIG(DISP_MODULE_ENUM module, void* cmdq, LCM_DSI_PARAMS *dsi_params)
{
	...
	if (0 != dsi_params->PLL_CLOCK) {
		...
		DISP_LOG_PRINT(ANDROID_LOG_INFO, "DSI", "[DISP] - LK - DSI_PHY_TIMCONFIG, Cycle Time = %d(ns), Unit Interval = %d(ns). , lane# = %d \n", cycle_time, ui, lane_no);
	} else {
		...
		switch (div1) {
			...
			default:
				dprintf(INFO,"div1 should be less than 4!!\n");
				div1 = 4;
				break;
		}
		...
		switch (div2) {
			...
			case 3:
				...
				break;
			default:
				dprintf(INFO,"div2 should be less than 4!!\n");
				div2 = 4;
				break;
		}
		...
		switch (pre_div) {
			...
			default:
				dprintf(INFO,"pre_div should be less than 4!!\n");
				pre_div = 4;
				break;
		}
		...
		switch (post_div) {
			...
			default:
				dprintf(INFO,"post_div should be less than 4!!\n");
				post_div = 4;
				break;
		}
		...
		switch (fbk_sel) {
			...
			default:
				dprintf(INFO,"fbk_sel should be less than 4!!\n");
				fbk_sel = 4;
				break;
		}
		...
		DISP_LOG_PRINT(ANDROID_LOG_INFO, "DSI", "[DISP] - LK - DSI_PHY_TIMCONFIG, Cycle Time = %d(ns), Unit Interval = %d(ns). div1 = %d, div2 = %d, fbk_div = %d, lane# = %d \n", cycle_time, ui, div1, div2, fbk_div, lane_no);
	}
...
void DSI_set_cmdq_V2(DISP_MODULE_ENUM module, void* cmdq, unsigned cmd, unsigned char count, unsigned char *para_list, unsigned char force_update)
{
	...
	for (d = DSI_MODULE_BEGIN(module); d <= DSI_MODULE_END(module); d++) {
		if (0 != DSI_REG[d]->DSI_MODE_CTRL.MODE) {
			...
			dprintf(INFO,"set cmdq in VDO mode in set_cmdq_V2\n");
			if (cmd < 0xB0) {
				if (count > 1) {
					vm_cmdq.LONG_PKT = 1;
...
int ddp_dsi_init(DISP_MODULE_ENUM module, void* cmdq)
{
	...
	//DSI_OUTREG32(cmdq, 0x14000000+0x118, 0xffffffff);
	//DSI_OUTREG32(cmdq, 0x1401b000+0x010, 0x00000002);

	//wisky gzy 170505 按mtk邮件修改，解决上电波形异常导致的开机黑条黑块问题	(已经还原)//wujie
	DSI_MASKREG32(cmdq, 0x1021581c, 0x300, 0x000200);	/* clear bit8 */
	DSI_MASKREG32(cmdq, 0x1021681c, 0x300, 0x000200);	/* clear bit8 */
...
{
	// TODO: this is not a good design, because we did nothing to dsi/mipi hw in ddp_dsi_init, but in ddp_dsi_deinit, we directly power off mipi/dsi
	// TODO: if we want to improve this, need to split init and config flow in mipitx init sequence.
	int i = 0;

	memset(&_dsi_context, 0, sizeof(_dsi_context));

	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		DSI_OUTREGBIT(cmdq,     DSI_INT_ENABLE_REG, DSI_REG[i]->DSI_INTEN,  CMD_DONE,   0);
		DSI_OUTREGBIT(cmdq,     DSI_INT_ENABLE_REG, DSI_REG[i]->DSI_INTEN,  RD_RDY,     0);
		DSI_OUTREGBIT(cmdq,     DSI_INT_ENABLE_REG, DSI_REG[i]->DSI_INTEN,  TE_RDY,     0);
		DSI_OUTREGBIT(cmdq,     DSI_INT_ENABLE_REG, DSI_REG[i]->DSI_INTEN,  EXT_TE,     0);
		DSI_OUTREGBIT(cmdq,     DSI_INT_ENABLE_REG, DSI_REG[i]->DSI_INTEN,  VM_DONE,    0);
	}
	...
	DSI_PHY_clk_switch(module, cmdq, 0);
...
int ddp_dsi_config(DISP_MODULE_ENUM module, disp_ddp_path_config *config, void *cmdq)
{
	...
	DISPCHECK("===>Pmaster:CLK SETTING :%d\n",
	          _dsi_context[0].dsi_params.PLL_CLOCK);
	...
	/*done:*/
	DSI_BackupRegisters(module,cmdq);
	return 0;
}
...
int ddp_dsi_power_on(DISP_MODULE_ENUM module, void *cmdq_handle)
{
	...
	if (!s_isDsiPowerOn) {
		...
		if (module == DISP_MODULE_DSI1 || module == DISP_MODULE_DSI1) {
			//ret += clk_prepare(ddp_clk_map[MM_CLK_DSI1_ENGINE]);
			//ret += clk_enable(ddp_clk_map[MM_CLK_DSI1_ENGINE]);
			//ret += clk_prepare(ddp_clk_map[MM_CLK_DSI1_DIGITAL]);
			//ret += clk_enable(ddp_clk_map[MM_CLK_DSI1_DIGITAL]);
			if (ret > 0) {
...
int ddp_dsi_power_off(DISP_MODULE_ENUM module, void *cmdq_handle)
{
	...
	if (!s_isDsiPowerOn) {
		if (module == DISP_MODULE_DSI0 || module == DISP_MODULE_DSI1) {
			//ret += clk_disable(ddp_clk_map[MM_CLK_DSI0_ENGINE];
			//ret += clk_unprepare(ddp_clk_map[MM_CLK_DSI0_ENGINE]);
			//ret += clk_disable(ddp_clk_map[MM_CLK_DSI0_DIGITAL]);
			//ret += clk_unprepare(ddp_clk_map[MM_CLK_DSI0_DIGITAL]);
			if (ret > 0) {
		...
		if (module == DISP_MODULE_DSI1 || module == DISP_MODULE_DSI1) {
			//ret += clk_disable(ddp_clk_map[MM_CLK_DSI1_ENGINE]);
			//ret += clk_unprepare(ddp_clk_map[MM_CLK_DSI1_ENGINE]);
			//ret += clk_disable(ddp_clk_map[MM_CLK_DSI1_DIGITAL]);
			//ret += clk_unprepare(ddp_clk_map[MM_CLK_DSI1_DIGITAL]);
			if (ret > 0) {
				DISP_LOG_PRINT(ANDROID_LOG_WARN, "DSI", "DSI1 power manager API return FALSE\n");
			}
		}
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		if (_dsi_context[i].dsi_params.mode == CMD_MODE) {
			// no need this on ROME, we will make dsi is in idle when ddp_dsi_stop() returns
...
static int ddp_dsi_polling_irq(DISP_MODULE_ENUM module, int bit, int timeout)
{
	...
	if ( timeout <= 0) {
		...
		while (cnt > 0) {
			...
			if (reg_val & bit) {
				DSI_OUTREG32((unsigned int)NULL, irq_reg_base, ~reg_val);
				break;
			}
			udelay(100);
		}
