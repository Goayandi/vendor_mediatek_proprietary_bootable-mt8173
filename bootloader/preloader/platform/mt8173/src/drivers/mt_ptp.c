...
#define VMIN_VAL		MV_TO_VAL(1000) /* XXX: for VCORE_AO & VCORE_PDN */
...
#define VCORE_CON9		0x0278		/* for Vcore */
#define VCORE_CON10		0x027A		/* for Vcore */
...
#else
#define for_each_det(det) for (det = &ptp_detectors[PTP_DET_VCORE_PDN]; det < (ptp_detectors + ARRAY_SIZE(ptp_detectors)); det++)
#endif
/**
	...
	if (0 == p->PTPINITEN) {
		...
		for (i = 0; i < ARRAY_SIZE(devinfo); i++) {
			...
			if (val[4] == devinfo[i].M_HW_RES4 && val[5] == devinfo[i].M_HW_RES5) {
				...
				val[3] = devinfo[i].M_HW_RES_3;			/* M_HW_RES3 */
...
static void ptp_init_det(struct ptp_det *det, struct ptp_devinfo *devinfo)
{
	...
	case PTP_DET_VCORE_AO:
		...
		/* det->VBOOT	= upmu_get_reg_value(0x036C); */
		pmic_read_interface(0x036C, &voltage, 0xFFFF, 0x0);
		det->VBOOT = voltage;
		/* get DVFS frequency table */
		det->num_freq_tbl = 2;
		det->freq_tbl[0] = 1780 * 100 / 1600; /* XXX: percentage, 800/800, dedicated for VCORE only */
	...
	case PTP_DET_VCORE_PDN:
		...
		/* det->VBOOT	= upmu_get_reg_value(0x027a); */
		pmic_read_interface(0x027a, &voltage, 0xFFFF, 0x0);
		det->VBOOT = voltage;
		ptp_error("[%s]: det->VBOOT %d\n", __func__, det->VBOOT);
		/* get DVFS frequency table */
		det->num_freq_tbl = 2;
		det->freq_tbl[0] = 1866 * 100 / 1866; /* XXX: percentage, 800/800, dedicated for VCORE only */
		det->freq_tbl[1] = 1600 * 100 / 1866; /* XXX: percentage, 600/800, dedicated for VCORE only */
...
void ptp_init(void)
{
	...
	u32 chip_sw_ver;
	U32 vcore_pmic = 0;
	U32 vcore_ptp_volt = 0;
	u32 axi_sel_clk_id = -1;
	...
	if (0 == ptp_devinfo.PTPINITEN) {
		...
		ptp_notice("This AP doesn't have PTP-OD's relative efuse\n");
		return;
	}

	/* If we find out this AP is E2 IC, don't tune Vcore down. */
	if (mt_get_chip_sw_ver() == CHIP_SW_VER_02) {
		chip_info = seclib_get_devinfo_with_index(4);
		chip_sw_ver = (chip_info >> 28) & 1;
		ptp_notice("E3 & later IC => chip_sw_ver = 1, E2 IC => chip_sw_ver = 0\n");
		ptp_notice("chip_sw_ver = %d\n", chip_sw_ver);

		/* E3 & later IC => chip_sw_ver = 1, E2 IC => chip_sw_ver = 0 */
		if (chip_sw_ver == 0)
			return;

	} else if (mt_get_chip_sw_ver() == CHIP_SW_VER_01) {
		ptp_notice("This is E1 IC\n");
	} else {
		ptp_notice("unknown IC version, need to query how to check IC version\n");
	}

	pmic_read_interface(0x027a, &vcore_pmic, 0xFFFF, 0x0);
	vcore_ptp_volt = VAL_TO_MV(vcore_pmic);
	ptp_notice("Vcore is tuning down from %dmV\n", vcore_ptp_volt);

	if (vcore_ptp_volt != 1125) {
		ptp_notice("Vcore is not 1125mV. PTPOD wouldn't tune Vcore voltage down.\n");
		return;
	}

	/* Set axi_sel_clk to 273Mhz */
	DRV_WriteReg32(CLK_CFG_0, (DRV_Reg32(CLK_CFG_0) & 0xFFFFFF00) | 0x00000001);

	/* pmic PWM */
	pmic_config_interface(0x26a, 1, 0x1, 8);


#ifdef PTP_VCORE_AO
	ptp_init_det(&ptp_detectors[PTP_DET_VCORE_AO], &ptp_devinfo);
#endif
	ptp_init_det(&ptp_detectors[PTP_DET_VCORE_PDN], &ptp_devinfo);

	/* TODO: FIXME, enable thermal CG - MT_CG_PERI_THERM */

	ptp_debug("TOP_DCMCTL = 0x%X\n", ptp_read(TOP_DCMCTL));
#ifdef PTP_VCORE_AO
	base_ops_set_phase(&ptp_detectors[PTP_DET_VCORE_AO], PTP_PHASE_INIT01);
	ptp_isr();

	mt_ptp_reg_dump();

	base_ops_set_phase(&ptp_detectors[PTP_DET_VCORE_AO], PTP_PHASE_INIT02);
	ptp_isr();

	mt_ptp_reg_dump();
#endif
	base_ops_set_phase(&ptp_detectors[PTP_DET_VCORE_PDN], PTP_PHASE_INIT01);
	ptp_isr();

	mt_ptp_reg_dump();

	base_ops_set_phase(&ptp_detectors[PTP_DET_VCORE_PDN], PTP_PHASE_INIT02);
	ptp_isr();

	mt_ptp_reg_dump();

	/* Set axi_sel_clk to 208MHz */
        DRV_WriteReg32(CLK_CFG_0, (DRV_Reg32(CLK_CFG_0) & 0xFFFFFF00) | 0x00000005);

	{
		unsigned int vcore_volt;
#ifdef PTP_VCORE_AO
		ptp_detectors[PTP_DET_VCORE_AO].volt_tbl_init2[0] =    (0 == ptp_detectors[PTP_DET_VCORE_AO].MDES
								     && 0 == ptp_detectors[PTP_DET_VCORE_AO].BDES
								     && 0 == ptp_detectors[PTP_DET_VCORE_AO].DCMDET
								     && 0 == ptp_detectors[PTP_DET_VCORE_AO].DCBDET
								     ) ? 0 : ptp_detectors[PTP_DET_VCORE_AO].volt_tbl_init2[0];
#endif
		ptp_detectors[PTP_DET_VCORE_PDN].volt_tbl_init2[0] =    (0 == ptp_detectors[PTP_DET_VCORE_PDN].MDES
								      && 0 == ptp_detectors[PTP_DET_VCORE_PDN].BDES
								      && 0 == ptp_detectors[PTP_DET_VCORE_PDN].DCMDET
								      && 0 == ptp_detectors[PTP_DET_VCORE_PDN].DCBDET
								      ) ? 0 : ptp_detectors[PTP_DET_VCORE_PDN].volt_tbl_init2[0];
#ifdef PTP_VCORE_AO
		vcore_volt = max(ptp_detectors[PTP_DET_VCORE_AO].volt_tbl_init2[0], ptp_detectors[PTP_DET_VCORE_PDN].volt_tbl_init2[0]);

		vcore_volt = (0 == vcore_volt) ? ptp_detectors[PTP_DET_VCORE_AO].VBOOT : vcore_volt;
#else
		vcore_volt = ptp_detectors[PTP_DET_VCORE_PDN].volt_tbl_init2[0]+4;

		vcore_volt = (0 == vcore_volt) ? ptp_detectors[PTP_DET_VCORE_PDN].VBOOT : vcore_volt;
#endif
		ptp_debug("PTP set volt: 0x%X\n", vcore_volt);
#ifdef PTP_VCORE_AO
		if (ptp_detectors[PTP_DET_VCORE_AO].VBOOT == ptp_detectors[PTP_DET_VCORE_PDN].VBOOT
		    && ptp_detectors[PTP_DET_VCORE_AO].VBOOT > vcore_volt) {
#else
		if (ptp_detectors[PTP_DET_VCORE_PDN].VBOOT > vcore_volt) {
#endif
			int i;
#ifdef PTP_VCORE_AO
			for (i = 1; (ptp_detectors[PTP_DET_VCORE_AO].VBOOT - i) >= vcore_volt; i++) {
				/* XXX: VCORE_AO >; VCORE_PDN */
				pmic_config_interface(0x27a, (ptp_detectors[PTP_DET_VCORE_PDN].VBOOT - i), 0x7F, 0);
				pmic_config_interface(0x36C, (ptp_detectors[PTP_DET_VCORE_AO].VBOOT - i), 0x7F, 0);

				ptp_debug("0x%X\n", (ptp_detectors[PTP_DET_VCORE_AO].VBOOT - i));
			}
#else
			for (i = 1; (ptp_detectors[PTP_DET_VCORE_PDN].VBOOT - i) >= vcore_volt; i++) {
				pmic_config_interface(0x27a, (ptp_detectors[PTP_DET_VCORE_PDN].VBOOT - i), 0x7F, 0);
				pmic_config_interface(0x278, (ptp_detectors[PTP_DET_VCORE_PDN].VBOOT - i), 0x7F, 0);
				ptp_debug("0x%X\n", (ptp_detectors[PTP_DET_VCORE_PDN].VBOOT - i));
			}
#endif
		}
	}
	...
	/* pmic auto */
	pmic_config_interface(0x26a, 0, 0x1, 8);

	pmic_read_interface(0x027a, &vcore_pmic, 0xFFFF, 0x0);
	axi_sel_clk_id = DRV_Reg32(CLK_CFG_0) & 0xFF;
	vcore_ptp_volt = VAL_TO_MV(vcore_pmic);

	ptp_notice("Right now\n");
	ptp_notice("Vcore is %dmV\n", vcore_ptp_volt);

	if (axi_sel_clk_id != 0x05)
		ptp_notice("axi_sel_clk isn't 208MHz. It becomes axi_sel_clk_id = %d\n", axi_sel_clk_id);
	else
		ptp_notice("axi_sel_clk is 208MHz\n");
